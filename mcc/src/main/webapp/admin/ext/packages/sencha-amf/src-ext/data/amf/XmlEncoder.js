...
Ext.define('Ext.data.amf.XmlEncoder', {
    ...
    statics: {
        ...
        generateFlexUID: function(id) {
            ...
            uid += "-";
            // add timestamp
            t = Number(new Date()).valueOf().toString(16).toUpperCase(); // get the String representation of milliseconds in hex format
            j = 0;
    ...
    encodeDate: function(date) {
        return "<date>" + (Number(date)).toString() + "</date>";
    },
    ...
    encodeGenericObject: function(obj) {
        ...
        for (i in obj) {
            if (i == "$flexType") {
                ...
                traits.push(this.encodeString(String(i)));
